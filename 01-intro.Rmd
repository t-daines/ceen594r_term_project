...
The Utah Department of Transportation (UDOT) has become concerned with this issue throughout the state. UDOT aims to have a method that accurately reports the expected queue length and wait time on any given on-ramp (that utilizes ramp metering) along I-15. This project will focus on estimating the queue length on one on-ramp; the ramp chosen for analysis is the northbound on-ramp to I-15 at Layton Parkway in Davis County, Utah. 

First, this report explains in brief detail the basic design and implementation of ramp meters. Next, the data collection performed on the Layton Parkway on-ramp will be discussed. Following that will be the interpretation and analysis of the data. Finally, the report will conclude with a summary of all items discussed. 
